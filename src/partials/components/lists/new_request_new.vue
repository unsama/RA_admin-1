                        ...
                        template(slot="tbody")
                            tr(v-for='(row, ind) in all')
                                ...
                                td
                                    ...
                                    button.btn.btn-sm.btn-info(data-toggle='modal' data-target='#push_modal' v-on:click='openBidsReq(row.id,row.createdAt)' style="margin-bottom: 5px;margin-right: 5px;") Change Driver
                                    button.btn.btn-sm.btn-danger(type="button" style="width: 71px;" v-on:click='cancelReq(row.id)') Cancel
        push_driver(v-bind:sel_req_id="assign_req_id_md",:req_time="reqestTime")
...
<script>
    ...
    import pushdriver from '../modals/push_driver.vue'
